...
const CrowdSale = ({
  ...
  const checkTokens = async () => {
    const myTokens = await CheckBalanceNT(
      web3,
      NTokenContract,
      CrowdSaleContract
    );
  ...
  const changeAmountSend = (e: any) => {
    if (e.target.value < 0) {
      ...
      setAmountGet(e.target.value * 15421);
    }
  };
  ...
  const changeAmountGet = (e: any) => {
    if (e.target.value < 0) {
      ...
      setAmountSend(e.target.value / 15421);
    }
  };
    ...
    <Box className={classes.mainCont}>
      ...
      <Box className={classes.inCont}>
        ...
        <Box className={classes.rightCont}>
          <Typography variant="caption">{balance.toFixed(4)} ETH</Typography>
        </Box>
        ...
        <Box className={classes.contToken}>
          <Box className={classes.leftCont}>
            <Typography variant="caption">
              Tokens left: {tokens.toFixed(2)}/100000
            </Typography>
          </Box>
          ...
          <Box className={classes.rightCont}>
            ...
            <Typography
              variant="caption"
              onClick={() => copyText(NTokenContract)}
