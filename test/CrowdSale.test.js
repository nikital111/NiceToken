...
describe("CrowdSale", function () {
  ...
  beforeEach(async function () {
    ...
    const CrowdSaleContract = await ethers.getContractFactory(
      "Crowdsale",
      acc1
    );
    CrowdSale = await CrowdSaleContract.deploy(
      10000,
      acc1.address,
      NToken.address
    );
  ...
  it("buy tokens", async function () {
    ...
    const tx = await CrowdSale.connect(acc2).buyTokens(acc2.address, {
      value: ethers.utils.parseEther(valETH),
    });
    ...
    expect(tx)
      ...
      .withArgs(
        acc2.address,
        acc2.address,
        ethers.utils.parseEther(valETH),
        ethers.utils.parseEther(tokens)
      );

    await expect(() => tx).to.changeEtherBalances(
      [acc2, acc1],
      [ethers.utils.parseEther(`-${valETH}`), ethers.utils.parseEther(valETH)]
    );
